...
| `/plainenglish`          | State the captain reply-shape contract - one paragraph, at most two sentences, leading with the ask or the answer, with evidence and options in a file the captain can open - which a prompt-submission hook puts in front of every reply; `config/plainenglish` set to `off` turns that per-turn reminder off without repealing the contract |
| `/updatefirstmate`       | Self-update the running firstmate and its secondmates to the latest from origin with fast-forward-only pulls, then re-read instructions and nudge secondmates |
| `/updatethecaptain`      | Keep the captain posted on every worker under way with a plain-English report - a dependency map of the whole body of work, then what each worker has finished, what it is doing now, what is left, and how long it needs - once immediately and then every ten minutes, surfacing any worker's question for the captain at the top of the message, until no worker is left running or the captain stops the updates |
| `/updatethecaptain-stop` | End the repeating worker report loop started by `/updatethecaptain`; harmless when no loop is running, and the workers themselves keep going |
